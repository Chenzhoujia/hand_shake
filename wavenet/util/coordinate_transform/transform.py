...
while(1):
    if angel%360==0:
        angel=0
        v4 = Vector((random.uniform(0,1), random.uniform(0,1), random.uniform(0,1)))
    R = Rotation(2*np.pi/360*angel, v4)
    angel = angel+5
    v5 = R(v1)
    ...
    #  将数据点分成三部分画，在颜色上有区分度
    ax.scatter(v1.co[0],v1.co[1],v1.co[2], c='r',s=100, label='star')  # 绘制数据点
    ax.plot([-v4.co[0]*10,v4.co[0]*10],[-v4.co[1]*10,v4.co[1]*10],[-v4.co[2]*10,v4.co[2]*10],label='axis',linewidth=3)
    ax.scatter(v5.co[0],v5.co[1],v5.co[2], c='g',s=100, label='end')
