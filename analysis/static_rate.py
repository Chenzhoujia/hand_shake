...
if is_RNN:
    ...
    delete_tremor = np.zeros((num_exampe, 3))

    for examp_i in range(num_exampe):

        wave_target = np.loadtxt("/media/chen/4CBEA7F1BEA7D1AE/Download/hand_dataset/pakinson/shake_tra_test_result/multi_stage/"
                   + "target/" + str(examp_i).zfill(5) + ".txt")
        wave_result = np.loadtxt("/media/chen/4CBEA7F1BEA7D1AE/Download/hand_dataset/pakinson/shake_tra_test_result/multi_stage/"
                   + "result/" + str(examp_i).zfill(5) + ".txt")

        wave_target = wave_target[180:, :]
        wave_result = wave_result[180:, :]
        for tmp_over1_i in range(len(wave_target)):
            for xyz_i in range(3):
                #累加振幅
                total_tremor[examp_i,xyz_i]+=abs(wave_target[tmp_over1_i,xyz_i])
                #消除的震颤部分——附加后震颤幅度变小：方向一致且幅度小于2×target
                if wave_target[tmp_over1_i,xyz_i]*wave_result[tmp_over1_i,xyz_i]>0:
                    if abs(wave_result[tmp_over1_i, xyz_i]) < abs(wave_target[tmp_over1_i, xyz_i]):
                        delete_tremor[examp_i,xyz_i]+=abs(wave_result[tmp_over1_i, xyz_i])
                    elif abs(wave_result[tmp_over1_i, xyz_i])/2 < abs(wave_target[tmp_over1_i, xyz_i]):
                        delete_tremor[examp_i,xyz_i]+=2*abs(wave_target[tmp_over1_i, xyz_i]) - abs(wave_result[tmp_over1_i, xyz_i])

        if np.mean(delete_tremor[examp_i, :]/total_tremor[examp_i, :])>0.5:
            print(total_tremor[examp_i, :])
            print(delete_tremor[examp_i, :])
        elif examp_i%100==0:
            print(examp_i)

    mean_rate = np.mean(delete_tremor/total_tremor)
